...
System.register("chunks:///_virtual/HotUpdateCore.ts", ['cc'], function (exports) {
  ...
  return {
    ...
    execute: function () {
      cclegacy._RF.push({}, "ffb54e9Ln9CiLOOdO0SHq0L", "HotUpdateCore", undefined);
      const jsb = globalThis.jsb;

      /**
       * 热更新结果
       */
      let HotUpdateResult = exports('HotUpdateResult', /*#__PURE__*/function (HotUpdateResult) {
        HotUpdateResult["AlreadyUpToDate"] = "AlreadyUpToDate";
        HotUpdateResult["Updated"] = "Updated";
        HotUpdateResult["Failed"] = "Failed";
        HotUpdateResult["ManifestError"] = "ManifestError";
       ...
       *
       * 策略：
       * - 首次热更新：下载 ZIP 包（单个请求，速度快）→ 解压到 storagePath
       * - 后续热更新：native.AssetsManager 差分更新（只下载变化的文件）
       *
       * 判断依据：storagePath/{bundleName}/project.manifest 是否存在
       * - 不存在 → 首次热更（ZIP 模式）
       * - 存在 → 后续热更（AssetsManager 差分模式）
       *
       * ZIP 包结构（由 pack-bundles.js 生成）：
       *   {bundleName}.zip
       *   ├── project.manifest   ← 解压后作为 AssetsManager 本地基准
       *   ├── version.manifest
       *   └── (bundle 资源文件)
       *
       * 流程图：
       *   APK v1.0.0 → 服务器 v1.1.0 → ZIP 下载 → storagePath 有 v1.1.0 manifest
       *   APK v1.0.0 → 服务器 v1.2.0 → AssetsManager 从 v1.1.0 差分到 v1.2.0
       */
      class HotUpdateCore {
        constructor(cdnBase) {
          /** 每个 bundle 的 AssetsManager 实例 */
          ...
          /** CDN 基础地址 */
          this._cdnBase = '';
          /** 保持 Downloader 引用，防止 GC 回收导致 native crash */
          this._downloader = null;
          this._cdnBase = cdnBase;
          if (sys.isNative) {
            const writablePath = jsb != null && jsb.fileUtils ? jsb.fileUtils.getWritablePath() : '/';
            this._storagePath = writablePath + 'remote-asset';
            console.log('[HotUpdate] 存储路径:', this._storagePath);
            console.log('[HotUpdate] CDN 地址:', this._cdnBase);
          }
        }
        get needRestart() {
         ...
         * @param manifestUrl APK 内置 manifest 的 nativeUrl（首次版本检测用）
         * @param onProgress 进度回调 (loaded, total, progress)
         */
        updateBundle(bundleName, manifestUrl, onProgress) {
          if (!sys.isNative) {
            console.log(`[HotUpdate] 非原生平台，跳过 ${bundleName} 热更新`);
            return Promise.resolve(HotUpdateResult.AlreadyUpToDate);
          }
          const storagePath = `${this._storagePath}/${bundleName}`;
          const localManifestPath = `${storagePath}/project.manifest`;

          // 判断本地是否已有热更版本
          const hasLocalHotUpdate = jsb.fileUtils.isFileExist(localManifestPath);
          if (hasLocalHotUpdate) {
            // 已有热更版本 → AssetsManager 差分更新
            console.log(`[HotUpdate] ${bundleName} 检测到本地热更版本，使用差分更新`);
            return this._diffUpdate(bundleName, storagePath, localManifestPath, onProgress);
          } else {
            // 首次热更 → 版本检测 + ZIP 下载
            console.log(`[HotUpdate] ${bundleName} 首次热更新，使用 ZIP 下载`);
            return this._zipUpdate(bundleName, manifestUrl, storagePath, onProgress);
          }
        }

        /**
         * 获取 bundle 的热更新存储路径（loadBundle 时使用）
         */
        getBundleStoragePath(bundleName) {
          return `${this._storagePath}/${bundleName}`;
        }

        /**
         * 获取 bundle 的正确加载 URL
         * 原生平台：优先使用热更目录，fallback 到包名（APK 内置）
         * Web：直接返回 bundleName
         */
        getBundleUrl(bundleName) {
          var _jsb$fileUtils;
          if (!sys.isNative) return bundleName;
          const storagePath = this.getBundleStoragePath(bundleName);
          if (jsb != null && (_jsb$fileUtils = jsb.fileUtils) != null && _jsb$fileUtils.isFileExist(`${storagePath}/project.manifest`)) {
            return storagePath;
          }
          return bundleName;
        }
        ...
        /**
         * 确保 bundle 可以加载（按需下载/更新）
         *
         * 流程：
         * 1. 已加载 → 直接返回
         * 2. 非原生平台 → 直接返回（Web 从 CDN 加载无需预处理）
         * 3. storagePath 有 manifest → 差分更新
         * 4. storagePath 无 manifest → 检查 CDN 是否有此 bundle → ZIP 下载
         *
         * @returns 加载 bundle 时应使用的 URL
         */
        async ensureBundleReady(bundleName, onProgress) {
          if (!sys.isNative) return bundleName;
          const storagePath = this.getBundleStoragePath(bundleName);
          const localManifestPath = `${storagePath}/project.manifest`;
          const hasLocal = jsb.fileUtils.isFileExist(localManifestPath);
          if (hasLocal) {
            // 本地有热更版本 → 尝试差分更新
            console.log(`[HotUpdate] ensureBundleReady: ${bundleName} 有本地版本，检查更新...`);
            try {
              await this._diffUpdate(bundleName, storagePath, localManifestPath, onProgress);
            } catch (e) {
              console.warn(`[HotUpdate] ${bundleName} 差分更新异常，使用本地版本:`, e);
            }
            return storagePath;
          } else {
            // 本地没有 → 检查 CDN 是否有此 bundle 的 version.manifest
            console.log(`[HotUpdate] ensureBundleReady: ${bundleName} 本地无文件，尝试下载...`);
            const hasRemote = await this._checkRemoteVersion(bundleName);
            if (hasRemote) {
              const result = await this._downloadAndExtractZip(bundleName, storagePath, onProgress);
              if (result === HotUpdateResult.Updated) {
                return storagePath;
              }
            }
            // 没有远程版本或下载失败 → fallback 到包名（APK 内置）
            return bundleName;
          }
        }

        /**
         * 通过 HTTP 检查 CDN 上是否存在 bundle 的 version.manifest
         * 替代 AssetsManager.checkUpdate()，避免 SIGSEGV crash
         */
        _checkRemoteVersion(bundleName) {
          return new Promise(resolve => {
            const url = `${this._cdnBase}/${bundleName}/version.manifest`;
            console.log(`[HotUpdate] 检查远程版本: ${url}`);
            const xhr = new XMLHttpRequest();
            xhr.onload = () => {
              if (xhr.status === 200) {
                try {
                  const manifest = JSON.parse(xhr.responseText);
                  console.log(`[HotUpdate] ${bundleName} 远程版本: ${manifest.version}`);
                  resolve(true);
                } catch {
                  resolve(false);
                }
              } else {
                console.log(`[HotUpdate] ${bundleName} 远程无此 bundle (${xhr.status})`);
                resolve(false);
              }
            };
        ...
        /**
         * 销毁所有 AssetsManager 实例
         */
        destroy() {
          for (const [name] of this._managers) {
            this._cleanupManager(name);
          }
        ...
        /**
         * 首次热更新：通过 HTTP 拉取 version.manifest 做版本比较，发现新版本后下载 ZIP
         *
         * 注意：不使用 native.AssetsManager.checkUpdate()，因为在 Android 上会导致 SIGSEGV crash
         */
        async _zipUpdate(bundleName, manifestUrl, storagePath, onProgress) {
          // 通过 HTTP 检查远程版本（避免 AssetsManager SIGSEGV crash）
          const remoteVersion = await this._fetchRemoteVersion(bundleName);
          if (!remoteVersion) {
            // 远程没有此 bundle 或网络错误
            console.log(`[HotUpdate] ${bundleName} 远程无可用版本`);
            return HotUpdateResult.AlreadyUpToDate;
          }

          // _zipUpdate 只在 storagePath 无文件时调用，所以无论版本是否一致都需要下载
          console.log(`[HotUpdate] ${bundleName} 远程版本 ${remoteVersion}，开始 ZIP 下载`);
          return this._downloadAndExtractZip(bundleName, storagePath, onProgress);
        }

        /**
         * 通过 HTTP 获取远程 version.manifest 的版本号
         */
        _fetchRemoteVersion(bundleName) {
          return new Promise(resolve => {
            const url = `${this._cdnBase}/${bundleName}/version.manifest`;
            const xhr = new XMLHttpRequest();
            xhr.onload = () => {
        ...
        /**
         * 下载 ZIP 包并解压到 storagePath
         */
        _downloadAndExtractZip(bundleName, storagePath, onProgress) {
          return new Promise(resolve => {
            const zipUrl = `${this._cdnBase}/${bundleName}/${bundleName}.zip`;
            const zipPath = `${this._storagePath}/${bundleName}.zip`;
            console.log(`[HotUpdate] 下载 ZIP: ${zipUrl}`);
            console.log(`[HotUpdate] 临时文件: ${zipPath}`);

            // 确保存储根目录存在
            jsb.fileUtils.createDirectory(this._storagePath);

            // 保存为类成员，防止 GC 回收导致 native crash
            this._downloader = new native.Downloader();
            this._downloader.onFileTaskSuccess = _task => {
              console.log(`[HotUpdate] ${bundleName} ZIP 下载完成，开始解压...`);

              // 确保解压目标目录存在
              jsb.fileUtils.createDirectory(storagePath);

              // 解压 ZIP
              const success = this._unzipFile(zipPath, storagePath);

              // 删除 ZIP 临时文件
              if (jsb.fileUtils.isFileExist(zipPath)) {
                jsb.fileUtils.removeFile(zipPath);
              }
              this._downloader = null;
              if (success) {
                console.log(`[HotUpdate] ${bundleName} ZIP 解压成功`);
                // 更新搜索路径
                this._addSearchPaths(bundleName, storagePath);
                // 首次 ZIP 下载不需要重启（脚本还未加载到内存）
                resolve(HotUpdateResult.Updated);
              } else {
                console.error(`[HotUpdate] ${bundleName} ZIP 解压失败`);
                // 清理残留
                if (jsb.fileUtils.isDirectoryExist(storagePath)) {
                  jsb.fileUtils.removeDirectory(storagePath);
                }
                resolve(HotUpdateResult.Failed);
              }
            };
            this._downloader.onTaskProgress = (_task, _bytesReceived, totalBytesReceived, totalBytesExpected) => {
              if (totalBytesExpected > 0 && onProgress) {
                const progress = totalBytesReceived / totalBytesExpected;
                onProgress(totalBytesReceived, totalBytesExpected, progress);
              }
            };
            this._downloader.onTaskError = (_task, errorCode, _errorCodeInternal, errorStr) => {
              console.error(`[HotUpdate] ${bundleName} ZIP 下载失败: [${errorCode}] ${errorStr}`);
              this._downloader = null;
              if (jsb.fileUtils.isFileExist(zipPath)) {
                jsb.fileUtils.removeFile(zipPath);
              }
            ...
            this._downloader.createDownloadFileTask(zipUrl, zipPath);
          });
        }
        ...
        /**
         * 差分更新：使用 native.AssetsManager 基于 storagePath 中的本地 manifest
         *
         * 参考旧项目 hotupdate.js 的做法：
         * 1. 先读取 manifest 文件内容为字符串
         * 2. 用 new jsb.Manifest(content, storagePath) 创建 Manifest 对象
         * 3. 调用 loadLocalManifest(manifest, storagePath) — 传入对象而非路径
         *
         * 这样 AssetsManager 基于内存中的 Manifest 对象工作，不会锁定/读取
         * storagePath 里的 manifest 文件，避免 update() 下载时写入冲突导致 SIGSEGV
         */
        async _diffUpdate(bundleName, storagePath, localManifestPath, onProgress) {
          console.log(`[HotUpdate] ${bundleName} 差分更新开始`);
          console.log(`[HotUpdate] 本地 manifest: ${localManifestPath}`);

          // 1. 先读取本地 manifest 内容
          const manifestContent = jsb.fileUtils.getStringFromFile(localManifestPath);
          if (!manifestContent) {
            console.error(`[HotUpdate] ${bundleName} 无法读取 manifest 文件: ${localManifestPath}`);
            return HotUpdateResult.ManifestError;
          }

          // 2. 解析本地版本，通过 HTTP 获取远程版本，先比较再决定是否更新
          //    避免 AssetsManager 内部 HTTP 客户端与 CDN 的兼容性问题
          ...
          const remoteVersion = await this._fetchRemoteVersion(bundleName);
          console.log(`[HotUpdate] ${bundleName} 本地版本: ${localVersion}, 远程版本: ${remoteVersion || '无'}`);
          if (!remoteVersion || this._versionCompare(localVersion, remoteVersion) >= 0) {
            console.log(`[HotUpdate] ${bundleName} 已是最新版本，跳过更新`);
            return HotUpdateResult.AlreadyUpToDate;
          }

          // 3. 版本不同，使用 AssetsManager 差分更新
          console.log(`[HotUpdate] ${bundleName} 需要从 ${localVersion} 更新到 ${remoteVersion}`);
          return new Promise(resolve => {
            const am = new native.AssetsManager('', storagePath, this._versionCompare);
            if (sys.os === sys.OS.ANDROID) {
              am.setMaxConcurrentTask(4);
            }
            am.setVerifyCallback((_filePath, asset) => {
              if (asset.compressed) return true;
              return true;
            });
            this._managers.set(bundleName, am);

            // 2. 用 manifest 内容字符串创建 Manifest 对象（参考旧项目 hotupdate.js:368）
            const manifest = new jsb.Manifest(manifestContent, storagePath);

            // 3. 传入 Manifest 对象 + storagePath（参考旧项目 hotupdate.js:369）
            am.loadLocalManifest(manifest, storagePath);
            if (!am.getLocalManifest() || !am.getLocalManifest().isLoaded()) {
              console.error(`[HotUpdate] ${bundleName} 本地 manifest 加载失败`);
              this._cleanupManager(bundleName);
              resolve(HotUpdateResult.ManifestError);
              return;
            }
            console.log(`[HotUpdate] ${bundleName} 本地版本: ${am.getLocalManifest().getVersion()}`);

            // 超时保护：AssetsManager 没有内置超时，网络不通会永远卡住
            let resolved = false;
            ...
            const timeoutId = setTimeout(() => {
              if (!resolved) {
                resolved = true;
                console.warn(`[HotUpdate] ${bundleName} 差分更新超时 (30s)，跳过更新使用本地版本`);
                this._cleanupManager(bundleName);
                resolve(HotUpdateResult.Failed);
              }
            }, 30000);
            ...
            am.setEventCallback(event => {
              ...
              switch (code) {
                case native.EventAssetsManager.ERROR_NO_LOCAL_MANIFEST:
                  console.error(`[HotUpdate] ${bundleName} 无本地 manifest`);
                  this._cleanupManager(bundleName);
                  safeResolve(HotUpdateResult.ManifestError);
                  break;
                case native.EventAssetsManager.ERROR_DOWNLOAD_MANIFEST:
                case native.EventAssetsManager.ERROR_PARSE_MANIFEST:
                  console.error(`[HotUpdate] ${bundleName} 远程 manifest 下载/解析失败`);
                  this._cleanupManager(bundleName);
                  safeResolve(HotUpdateResult.Failed);
                  break;
                case native.EventAssetsManager.ALREADY_UP_TO_DATE:
                  console.log(`[HotUpdate] ${bundleName} 已是最新版本`);
                  this._cleanupManager(bundleName);
                  safeResolve(HotUpdateResult.AlreadyUpToDate);
                  break;
                case native.EventAssetsManager.NEW_VERSION_FOUND:
                  console.log(`[HotUpdate] ${bundleName} 发现新版本，开始差分更新...`);
                  console.log(`[HotUpdate] 需下载: ${am.getTotalBytes()} bytes, ${am.getTotalFiles()} 个文件`);
                  am.update();
                  break;
                case native.EventAssetsManager.UPDATE_PROGRESSION:
                  {
                    ...
                    const progress = isNaN(percentByFile) ? 0 : percentByFile;
                    onProgress == null || onProgress(downloadedBytes, totalBytes, progress);
                    const msg = event.getMessage();
                    if (msg) {
                      console.log(`[HotUpdate] ${bundleName} ${msg}`);
                    }
                    break;
                  }
                case native.EventAssetsManager.UPDATE_FINISHED:
                  console.log(`[HotUpdate] ${bundleName} 差分更新完成`);
                  this._onAssetsManagerUpdated(bundleName, am);
                  this._cleanupManager(bundleName);
                  safeResolve(HotUpdateResult.Updated);
                  break;
                case native.EventAssetsManager.UPDATE_FAILED:
                  failRetryCount++;
                  console.error(`[HotUpdate] ${bundleName} 更新失败 (${failRetryCount}/${MAX_FAIL_RETRY}): ${event.getMessage()}`);
                  if (failRetryCount < MAX_FAIL_RETRY) {
                    am.downloadFailedAssets();
                  } else {
                    console.error(`[HotUpdate] ${bundleName} 重试次数耗尽，放弃更新`);
                    this._cleanupManager(bundleName);
                    safeResolve(HotUpdateResult.Failed);
                  }
                  break;
                case native.EventAssetsManager.ERROR_UPDATING:
                  console.error(`[HotUpdate] ${bundleName} 文件更新错误: ${event.getAssetId()} - ${event.getMessage()}`);
                  break;
                case native.EventAssetsManager.ERROR_DECOMPRESS:
                  console.error(`[HotUpdate] ${bundleName} 解压错误: ${event.getMessage()}`);
                  break;
              }
            });
            am.checkUpdate();
        ...
        /**
         * AssetsManager 差分更新完成后的处理
         */
        _onAssetsManagerUpdated(bundleName, am) {
          const newSearchPaths = am.getLocalManifest().getSearchPaths();
          this._saveSearchPaths(bundleName, newSearchPaths);
         ...
         * ZIP 解压后添加搜索路径
         */
        _addSearchPaths(bundleName, storagePath) {
          // ZIP 模式：storagePath 本身就是搜索路径
          const newPaths = [storagePath + '/'];
          this._saveSearchPaths(bundleName, newPaths);
        }

        /**
         * 合并并保存搜索路径
         */
        _saveSearchPaths(bundleName, newPaths) {
          console.log(`[HotUpdate] ${bundleName} 新搜索路径:`, JSON.stringify(newPaths));

          // 合并到全局搜索路径（热更路径放最前面，优先搜索）
          const currentPaths = jsb.fileUtils.getSearchPaths();
          const mergedPaths = [...newPaths, ...currentPaths.filter(p => !newPaths.includes(p))];
          jsb.fileUtils.setSearchPaths(mergedPaths);

          // 保存到 localStorage（main.js 启动时恢复）
          const savedPaths = sys.localStorage.getItem('HotUpdateSearchPaths');
          let allPaths = savedPaths ? JSON.parse(savedPaths) : [];
          for (const p of newPaths) {
            if (!allPaths.includes(p)) {
              allPaths.unshift(p);
            }
          }
          sys.localStorage.setItem('HotUpdateSearchPaths', JSON.stringify(allPaths));
        }
        ...
        /**
         * 解压 ZIP 文件
         * Android: 通过 native.reflection 调用 Java UnzipHelper
         * iOS: 通过 native.reflection 调用 ObjC NSFileManager + SSZipArchive
         */
        _unzipFile(zipPath, outPath) {
          try {
            if (sys.os === sys.OS.ANDROID) {
              // 调用 Java: UnzipHelper.unzipFile(zipPath, outPath)
              return native.reflection.callStaticMethod('com/cocos/game/UnzipHelper', 'unzipFile', '(Ljava/lang/String;Ljava/lang/String;)Z', zipPath, outPath);
            }

            // iOS / 其他平台 fallback
            if (jsb.zipUtils && typeof jsb.zipUtils.unzipFile === 'function') {
              return jsb.zipUtils.unzipFile(zipPath, outPath);
            }
            console.error('[HotUpdate] 当前平台未实现 ZIP 解压');
            return false;
          } catch (e) {
            console.error('[HotUpdate] ZIP 解压异常:', e);
            return false;
          }
        }
        _cleanupManager(key) {
          const am = this._managers.get(key);
          if (am) {
        ...
        /**
         * 版本比较
         * 支持格式: "1.0.1" 或 "1.0.1.a3b2c1d0"（内容哈希后缀）
         * 前 3 段数字比较，第 4 段（内容哈希）字符串比较
         * 返回: >0 表示 A 更新, 0 表示相同, <0 表示 B 更新
         */
        _versionCompare(versionA, versionB) {
      ...
      exports('HotUpdateCore', HotUpdateCore);
      cclegacy._RF.pop();
    }
  };
});
  ...
  return {
    ...
    execute: function () {
      ...
      let LaunchScene = exports('LaunchScene', (_dec = ccclass('LaunchScene'), _dec2 = property(Asset), _dec3 = property(Asset), _dec(_class = (_class2 = class LaunchScene extends Component {
        ...
        getBundleDisplayName(bundleName) {
          const nameMap = {
            'lobby-casino-core': '核心包',
            'lobby-casino': '大厅',
            'lobby-casino-email': '邮件系统'
          };
          return nameMap[bundleName] || bundleName;
        }
        delay(ms) {
...
System.register("chunks:///_virtual/main", ['./Config2.ts', './HotUpdateCore.ts', './LaunchScene.ts', './MockData.ts', './ProgressBarTarget.ts', './Splash.ts', './Types.ts', './FileUtils.ts', './Md5.ts', './GGHotUpdateInstance.ts', './GGHotUpdateManager.ts', './GGHotUpdateType.ts', './GGJsb.ts', './GGZip.ts', './GGZipTypes.ts', './GGEventManager.ts', './GGLogger.ts', './GGObserverSystem.ts'], function () {
  return {
